...
def image_noise(n, im_size, device):
    if isinstance(im_size, (tuple, list)):
...
class Dataset(data.Dataset):
    def __init__(self, folder, image_size, transparent = False, aug_prob = 0.):
        ...
        self.transform = transforms.Compose([
            ...
            RandomApply(aug_prob, transforms.RandomResizedCrop(image_size, scale=(0.5, 1.0), ratio=(image_size[1]//image_size[0], image_size[1]//image_size[0])), transforms.CenterCrop(image_size)),
            transforms.ToTensor(),
...
class Generator(nn.Module):
    def __init__(self, image_size, latent_dim, network_capacity = 16, transparent = False, attn_layers = [], no_const = False, fmap_max = 512):
        ...
        self.num_layers = int(log2(max(image_size)) - 1)
        ...
        if no_const:
            ...
            self.initial_block = nn.Parameter(torch.randn((1, init_channels, 4 // aspect_ratio, 4)))
...
class Discriminator(nn.Module):
    def __init__(self, image_size, network_capacity = 16, fq_layers = [], fq_dict_size = 256, attn_layers = [], transparent = False, fmap_max = 512):
        ...
        if not isinstance(image_size, (tuple, list)):
            image_size = (image_size, image_size)
        num_layers = int(log2(max(image_size)) - 1)
        ...
        chan_last = filters[-1]
        aspect_ratio = image_size[1] // image_size[0]
        latent_dim = 2 * 2 * chan_last // aspect_ratio
    ...
    def __init__(
        ...
        **kwargs
        ...
        if isinstance(image_size, (tuple, list)):
            assert log2(image_size[0]).is_integer() and log2(image_size[1]).is_integer(), 'image size must be a power of 2 (64, 128, 256, 512, 1024)'
        else:
...
class ModelLoader:
    ...
    def mixing_style(self):
        def tile(a, dim, n_tile):
            init_dim = a.size(dim)
            repeat_idx = [1] * a.dim()
            repeat_idx[dim] = n_tile
            a = a.repeat(*(repeat_idx))
            order_index = torch.LongTensor(np.concatenate([init_dim * np.arange(n_tile) + i for i in range(init_dim)])).cuda(self.model.rank)
            return torch.index_select(a, dim, order_index)
        ...
        tmp1 = torch.randn(num_class, latent_dim).cuda()
        tmp1 = tile(tmp1, 0, sample_per_class)
        # tmp1 = tmp1.repeat(num_rows, 1)
        tmp2 = torch.randn(sample_per_class, latent_dim).cuda()
        tmp2 = tmp2.repeat(num_class, 1)

        tt = int(num_layers / 1)
        mixed_latents = [(tmp1, tt), (tmp2, num_layers - tt)]
        img_noise = image_noise(num_class*sample_per_class, image_size, device=0)
        # img_noise = image_noise(1, image_size, device=0)
        # img_noise = img_noise.repeat(num_class*sample_per_class, 1, 1, 1)

        generated_images = self.model.generate_truncated(self.model.GAN.SE, self.model.GAN.GE, mixed_latents, img_noise, trunc_psi = 0.8)
        # print(generated_images.shape)
        torchvision.utils.save_image(generated_images[:40], "./sample.bmp", nrow=sample_per_class)
        for i in range(generated_images.shape[0]):
            ndarr = generated_images[i].mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu', torch.uint8).numpy()[:, :, 0]
            im = Image.fromarray(ndarr)
            im.save("/home/alien/ouweifeng/stylegan2-pytorch/stylegan2_pytorch/samples_6000_300_20/%05d.bmp"% (i+1))
        return